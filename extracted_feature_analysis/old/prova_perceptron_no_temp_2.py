...
X_train = torch.from_numpy(features_train.reshape(-1, 5, 1024).reshape(-1, 5120))
...
X_test = torch.from_numpy(features_test.reshape(-1, 5, 1024).reshape(-1, 5120))
...
model = SimpleMLP(input_dim= 5120, num_classes= len(set(labels_train)))
